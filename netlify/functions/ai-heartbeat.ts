...
export default async () => {
  ...
  if (!url || !key) return new Response(JSON.stringify({ok:false,reason:'AI heartbeat server key is not configured yet'}),{status:503,headers:{'content-type':'application/json'}})
