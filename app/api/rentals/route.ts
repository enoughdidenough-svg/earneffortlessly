...
export async function POST(req:Request){
 ...
 return req.headers.get('content-type')?.includes('form')?NextResponse.redirect(new URL('/buyer/rentals',req.url)):NextResponse.json({ok:true,session,message:'Rental is pending payment. Access is granted only after the related payment is verified.'});
}
